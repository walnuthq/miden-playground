...
const {
	...
	Word,
	MockWebClient: WasmMockWebClient // Alias the WASM-exported WebClient
} = wasm;
...
export class MockWebClient {
	constructor(rpcUrl, seed) {
		...
		this.wasmWebClient = new WasmMockWebClient();
	}
	 ...
	 * @param {Uint8Array} [seed] - The seed for the account.
	 * @returns {Promise<MockWebClient>} The fully initialized WebClient.
	 */
	static async createClient(rpcUrl, seed) {
		// Construct the instance (synchronously).
		const instance = new MockWebClient(rpcUrl, seed);
	...
	/* async newMintTransactionRequest(targetAccountId, faucetId, noteType, amount) {
		console.log('OK');
		return await this.wasmWebClient.newMintTransactionRequest(
			targetAccountId,
			faucetId,
			noteType,
			amount
	...
	async newTransaction(accountId, transactionRequest) {
		try {
			if (!this.worker) {
				return await this.wasmWebClient.newTransaction(accountId, transactionRequest);
			}
	...
	async submitTransaction(transactionResult, prover = undefined) {
		try {
			if (!this.worker) {
				return await this.wasmWebClient.submitTransaction(transactionResult, prover);
			}
	...
	async syncState() {
		try {
			...
			const serializedSyncSummaryBytes = await this.callMethodWithWorker(MethodName.SYNC_STATE);
			return wasm.SyncSummary.deserialize(new Uint8Array(serializedSyncSummaryBytes));
		} catch (error) {
