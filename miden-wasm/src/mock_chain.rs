...
impl MockChain {
    ...
    pub fn new() -> Self {
        Self {
            ...
            accounts: SimpleSmt::<{ ACCOUNT_TREE_DEPTH }>::new()
                .expect("depth too big for SimpleSmt"),
    ...
    pub fn seal_block(&mut self, block_num: Option<u32>) -> Block {
        ...
        let timestamp = previous.map_or(TIMESTAMP_START, |block| {
            block.header().timestamp() + TIMESTAMP_STEP
        });
