...
The first two inputs are the target account ID suffix and prefix, you can use the <Link href={`/accounts/${props.address}`}>Test Wallet</Link> account ID suffix **{props.accountIdSuffix}** and prefix **{props.accountIdPrefix}**.

The third input is the timelock block height, you can set it to any future block height eg. **{props.syncHeight}**.
