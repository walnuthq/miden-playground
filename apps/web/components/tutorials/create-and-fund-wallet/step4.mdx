...
A [Transaction](https://docs.miden.xyz/core-concepts/protocol/transaction) in Miden is the state transition of a single account. A Transaction takes as input a single account and zero or more notes, and outputs the same account with an updated state, together with zero or more notes.
...
In Miden, minting 100 MDN tokens from a fungible faucet to a wallet takes two transactions, one in which the faucet creates a note containing 100 MDN and one in which the wallet consumes that note and receives the 100 MDN.
