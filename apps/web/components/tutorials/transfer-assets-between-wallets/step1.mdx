...
- <Link href={`/accounts/${props.walletA.identifier}`}>Wallet A</Link>: This is
  the source wallet, it has been funded with MDN tokens and you will transfer
  some out to the destination wallet.
- <Link href={`/accounts/${props.walletB.identifier}`}>Wallet B</Link>: This is
  the destination wallet, it will receive MDN tokens from the source account.
- <Link href={`/accounts/${props.faucet.identifier}`}>MDN Faucet</Link>: This is
